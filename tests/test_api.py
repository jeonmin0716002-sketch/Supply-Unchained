...
from api.package_fetcher import PackageFetchError
...
def _patch_layers(monkeypatch, *, vulns=(), findings=(), signals=SAFE_SIGNALS, score=5):
    ...
    async def fake_static(req, http, errors):
        return list(findings)

    async def fake_scoring(req, errors):
...
def test_static_layer_converts_fetch_error(client, monkeypatch):
    """package_fetcher 예외가 스캔 실패가 아니라 layer_errors 로 강등되는지."""

    class _Boom:
        def __init__(self, *a, **k): ...
        async def __aenter__(self):
            raise PackageFetchError("nope")
        async def __aexit__(self, *a):
            return False

    monkeypatch.setattr(scan, "fetched_package_tree", _Boom)
